...
#mini_dm_dataset = ['clipart', 'painting', 'real', 'Sketch']
dm_dataset = ['clipart', 'painting', 'real', 'Sketch', 'quickdraw', 'infograph']
available_datasets = pacs_dataset + officehome_dataset + digits_datset+ dm_dataset
...
def get_train_dataloader(source_list=None, batch_size=64, image_size=224, crop=False, jitter=0, args=None, config=None):
    ...
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=False, drop_last=True)
    return loader

def get_train_dc_dataloader(source_list=None, batch_size=64, image_size=224, crop=False, jitter=0, args=None, config=None):
    if args is not None:
        source_list = args.source
    if config is not None:
        batch_size = config["batch_size"]
    ...
    for dname in source_list:
        datalists_folder = get_datalists_folder(args)
        path = os.path.join(datalists_folder, '%s_train.txt' % dname)
        train_dataset = get_dc_dataset(args=args,
                                    path=path,
                                    train=True,
                                    image_size=image_size,
                                    crop=crop,
